...
export function BlogCard({ post, className }: BlogCardProps) {
  ...
  return (
    <article
      className={cn(
        'group flex flex-col overflow-hidden rounded-2xl border border-[#E1E8EA] bg-white shadow-2xs transition-all duration-300 hover:shadow-xs hover:border-[#0F8F83]/40 h-full',
        className
      )}
    >
      <Link to={articleLink} className="flex flex-col h-full">
        {/* Article Cover Image (16:10 Standardized Ratio) */}
        <div className="relative aspect-[16/10] overflow-hidden bg-[#EEF6F8] w-full shrink-0">
          <SmartImage
            src={imageUrl}
            alt={post.title}
          ...
          <div className="absolute left-3.5 top-3.5 z-10 flex items-center gap-2">
            <span className="rounded-full bg-white/95 backdrop-blur-xs border border-[#E1E8EA] px-3 py-1 font-semibold text-[#111111] text-[9px] uppercase tracking-widest shadow-2xs">
              {post.category}
            </span>
          </div>
        </div>
        ...
        <div className="p-5 sm:p-6 flex-1 flex flex-col justify-between space-y-4">
          ...
          <div className="pt-4 border-t border-[#E1E8EA] flex items-center justify-between">
            ...
            <span className="flex items-center gap-1 text-xs font-semibold text-[#111111] group-hover:text-[#0F8F83] group-hover:translate-x-1 transition-all">
              Read <ArrowRight className="size-3.5 text-[#0F8F83]" />
            </span>
          </div>
        </div>
      </Link>
